<?php
    ...
    session_start();
    $customChamp = mysqli_real_escape_string($conexao, $_GET['customChamp']);
    $hashtag = mysqli_real_escape_string($conexao, $_GET['nomeUnicoCampeonato']);
    require_once "restrict/navbar.php";
?>

<div class="container">
    <div class="container2">
        <h2 class="text-uppercase especialpadd"><?=$customChamp?> (#<?=$hashtag?>)</h2>

        <?php
            $sql = "SELECT * FROM videos WHERE descricao LIKE '%".$hashtag."%' ORDER BY id DESC";
            ...
            <div class="row">
                ...
                <form method="POST" action="restrict/like.php">
                    <h3 class="video-data like-button">
                        ...
                        <input type="hidden" value="champ?customChamp=<?=$customChamp?>&nomeUnicoCampeonato=<?=$hashtag?>" name="headerlocal">
